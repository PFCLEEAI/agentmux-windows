...
- ConPTY session disposal closes the pane's input pipe, waits briefly for the direct child process to exit cooperatively, and kills only that direct child if it is still running after the bounded grace window. This keeps terminal pane shutdown bounded, but it is not process-tree cleanup, Windows Job Object containment, or live terminal process restore.
