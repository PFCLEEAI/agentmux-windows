...
CI publishes a lightweight framework-dependent Windows artifact after the Windows smoke gates pass. The package includes the WPF app at the root, the CLI under `cli\`, runtime config, dependency files, WebView2/xterm assets, package provenance metadata, `EVIDENCE.json`, SHA-256 checksums, README, license, a no-admin user install helper, and the manual desktop-smoke runbook/helper. CI also runs `cli\agentmux.exe --help` from the packaged output.
