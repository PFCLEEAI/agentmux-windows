...
- Browser automation uses the active browser pane and WebView2 directly: `surface.eval_js`, `surface.click_selector`, `surface.fill_selector`, `surface.browser_type_text`, `surface.browser_press_key`, `surface.capture_screenshot`, `surface.browser_frame_tree`, `surface.browser_network_log`, and `surface.browser_clear_network_log`. These are local trust-boundary APIs; JavaScript executes in the active browser pane, click/type/press use WebView2 input automation where practical, screenshots write to local paths, frame-tree preview returns the WebView2/CDP `Page.getFrameTree` shape for inspection, and network-log preview records compact CDP Network event metadata in memory. The network log avoids headers, cookies, request/response bodies, and post data, but it does include URLs, which may carry sensitive query data. Selector actions can optionally target a same-origin `iframe`/`frame` by top-document `name` or `id`; cross-origin frame automation, request interception/mutation, response body/header capture, HAR export, telemetry, and full Playwright-style browser control are intentionally out of scope for this preview.
