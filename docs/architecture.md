...
- Browser automation uses the active browser pane and WebView2 directly: `surface.eval_js`, `surface.click_selector`, `surface.fill_selector`, `surface.browser_type_text`, `surface.browser_press_key`, `surface.capture_screenshot`, and `surface.browser_frame_tree`. These are local trust-boundary APIs; JavaScript executes in the active browser pane, click/type/press use WebView2 input automation where practical, screenshots write to local paths, and frame-tree preview returns the WebView2/CDP `Page.getFrameTree` shape for inspection only.
