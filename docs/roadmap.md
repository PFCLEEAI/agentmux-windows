...
- Workspace switching. Preview implemented with `workspace.list`, `workspace.create`, `workspace.select`, `workspace.set_pull_request`, `workspace.set_ports`, `workspace.log`, `workspace.list_log`, `workspace.clear_log`, `workspace.set_status`, `workspace.list_status`, `workspace.clear_status`, `workspace.set_progress`, `workspace.clear_progress`, compact workspace DTOs, index/id selection, sidebar selected-state updates, render switching, lightweight `.git/HEAD` branch metadata, explicit persisted PR annotations, explicit persisted port annotations, latest unread notification preview metadata, in-memory latest-log/status/progress metadata, and hosted app-smoke coverage.
...
- Workspace set-status/list-status/clear-status. Preview implemented as explicit local in-memory workspace context, keyed per workspace and capped at 100 entries, with latest-status sidebar metadata and hosted app-smoke coverage. Automatic agent/process/task status inference, file/process/Event Log ingestion, watchers, and persistent status history remain out of scope.
- Workspace set-progress/clear-progress. Preview implemented as explicit local in-memory workspace context, one value per workspace, with latest-progress sidebar metadata and hosted app-smoke coverage. Automatic progress inference from terminal output, files, tests, git, processes, ports, Event Log, watchers, and persistent progress history remain out of scope.
...
- Back/forward/reload/current URL. Preview implemented through active-pane `browser.back`, `browser.forward`, `browser.reload`, and `browser.url.get` / `agentmux browser back|forward|reload|url|get-url`, without CMUX surface targeting, `--snapshot-after`, browser-history export, or persistent browser navigation history.
- Readable text extraction. Preview implemented through bounded active-pane rendered text extraction with optional selector and same-origin frame targeting.
- Element data extraction. Preview implemented through active-pane `agentmux browser get title|url|text|html|value|attr|count|box|styles`, returning capped text-like fields, selector counts, and WebView viewport CSS-pixel boxes without CMUX surface targeting.
- Element state checks. Preview implemented through active-pane `agentmux browser is visible|enabled|checked`, returning immediate DOM/style/property/ARIA boolean state without waits, retries, accessibility snapshots, or CMUX surface targeting.
- Element find helpers. Preview implemented through active-pane `agentmux browser find role|text|label|placeholder|testid|first|last|nth`, returning capped best-effort match metadata for inspection only, without stable locator refs, accessibility snapshots, waits, retries, or CMUX surface targeting.
- DOM snapshot preview. Preview implemented through active-pane `agentmux browser snapshot [--interactive] [--cursor] [--compact] [--max-depth <count>] [--selector <css>]`, returning capped top-document DOM-derived line text plus structured nodes without frame traversal, stable refs, accessibility-tree parity, form-value scraping, CMUX surface targeting, or `--snapshot-after`.
- Dropdown selection. Preview implemented through active-pane `agentmux browser select <selector> <value>`, selecting one HTML `<select>` option by exact value and dispatching DOM input/change events without CMUX surface targeting, `--snapshot-after`, multi-select values, label matching, or custom combobox support.
- Eval/hover/focus/click/fill/type/press. Preview implemented through lightweight WebView2 helpers; hover/click/type/press use WebView2 input automation where practical, focus uses DOM focus on the matched element, and hover/focus/click/fill/type/press can target same-origin child frames by name or id.
...
Browser automation has hosted WebView2 runtime smoke and CI PNG artifacts. Active-pane browser navigation now covers open, back, forward, reload, and current-url inspection without history export. Browser text preview covers capped rendered text extraction for the active document, selector, and same-origin frame without session-snapshot persistence. Browser get preview covers title, URL, text, HTML, value, attribute, count, box, and computed-style property extraction from the active pane. Browser is preview covers immediate visible/enabled/checked DOM state checks without session-snapshot persistence. Browser find preview covers role/text/label/placeholder/`data-testid` and first/last/zero-based nth lookup with capped inspection-only metadata and no stable element refs. Browser snapshot preview covers capped top-document DOM-derived line text and structured nodes without session-snapshot persistence. Browser select preview covers exact-value HTML `<select>` mutation with DOM input/change dispatch and no session-snapshot persistence. Hover/focus/click/type/press now cover richer pointer/focus/key preview semantics, same-origin frame-targeted selector actions cover the first actionable frame-parity step, frame-tree inspection exposes the active pane's frame structure, wait-for-selector covers one-shot DOM readiness synchronization, wait-load covers bounded active-pane `domcontentloaded`/`load`/preview `network-idle` readiness, console-log preview captures recent capped JavaScript console messages without session-snapshot persistence, network-log preview captures recent request/response/failure metadata without headers or bodies by default, response-body preview explicitly retrieves one capped request body by request id, metadata-only HAR-like preview exports retained network metadata without headers/cookies/post data/body text, trace preview writes bounded active-pane CDP trace JSON without returning raw trace data over RPC, download-log preview captures completed download metadata plus deterministic local file output, and route preview covers transient block/fulfill rules for the active pane. Full Playwright-style automation such as CMUX surface targeting, `--snapshot-after`, browser history export, cross-origin frame control, accessibility-tree snapshots, locator engines, stable element references, multi-select values, select-by-label/text/index, custom combobox widgets, automatic form scraping, full browser-context load-state/network-idle parity, persistent selector watches, browser-context routing, automatic response/header/cookie capture, full HAR export, full browser-context tracing, log streaming, source-map expansion, malware scanning, pause/resume/cancel download control, and trusted physical input remains future work.
