...
AgentMux saves `%LOCALAPPDATA%\AgentMux\session.json` and restores workspaces, active workspace, explicit workspace port annotations, surfaces, active surface, split layout, active pane, pane titles, browser URLs, and last terminal screen text. It does not restore live terminal processes; restored terminal panes start fresh ConPTY sessions as needed.
...
Workspace commands operate on app-level workspace state. `workspace list` returns compact workspace metadata for each workspace, including id, title, zero-based index, active flag, working directory, detected git branch, manually set port annotations, unread count, latest unread notification preview, surface count, active surface index/title, active pane id, pane count, and browser pane count; it does not return the full raw workspace/surface/pane object graph. Git branch detection reads the nearest `.git/HEAD` or `.git` indirection file from the workspace working directory without invoking `git`; detached HEAD, unreadable metadata, dirty status, live refresh, and automatic port discovery are intentionally not reported. Latest notification previews are whitespace-compacted, capped display strings for sidebar/API context only; notification bodies may still contain sensitive content and are visible to same-user local callers. `workspace create --title <name> --cwd <path>` creates and selects a new workspace, and `workspace select` accepts either zero-based `--index`, positional index, or `--id`. `workspace ports 3000 5173`, `workspace ports --ports 3000,5173`, and `workspace ports clear` set or clear explicit numeric port annotations for the active workspace; `--index <n>` or `--id <workspace-id>` can target another workspace. Port annotations are sorted, deduplicated, stored in the session snapshot, and shown as sidebar context, but they may be stale and do not prove a listener exists, is reachable, is healthy, or is owned by that workspace. `workspace ls`, `workspace new`, `workspace use`, and `workspace port` are accepted aliases. Workspace selection updates the WPF sidebar, active surface render, and saved session state.
